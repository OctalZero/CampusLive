...
void WindowsControl::ForwardServerInfo(const QJsonObject& data)
{
	...
	case MessageForwordControl::Login:
        if(login_control_)login_control_->ParseServerInfo(data);
		break;
	case MessageForwordControl::Register:
        if(register_control_)register_control_->ParseServerInfo(data);
		break;
	case MessageForwordControl::Homepage:
        if(homepage_control_)homepage_control_->ParseServerInfo(data);
		break;
	case MessageForwordControl::Classroom:
        if(classroom_control_)classroom_control_->ParseServerInfo(data);
		break;
	default:
		break;
	}
