	...
	: homepage_window_(new HomepageWindow())
{
    ...
    connect(homepage_window_, &HomepageWindow::SendCreateCourse, this, &HomepageControl::SendCreateCourse);
...
void HomepageControl::ParseServerInfo(const QJsonObject& data)
{
	...
	case Homepage::InitHomepageResult:
		...
		break;
    case Homepage::RfreshCourseStatusResult:
        if (homepage_window_) {  // 将解析的数据进行显示
            homepage_window_->UpdateCourseInfo(info["courses_info"].toArray());
        }
        break;
    case Homepage::CreateCourseResult:
		ParseAddCourseResult(info);
		break;
    case Homepage::DeleteCourseResult:
...
void HomepageControl::SendCreateCourse(const QJsonObject& data)
{
	QJsonObject info, info_data;
	info["to"] = MessageForwordControl::Homepage;
    info["type"] = Homepage::CreateCourse;
    info_data["course_name"] = data["course_name"];
	info["data"] = info_data;
	qDebug() << info;
...
void HomepageControl::SendDeselectionCourse(const QJsonObject& data)
{
	QJsonObject info, info_data;
	info["to"] = MessageForwordControl::Homepage;
    info["type"] = Homepage::DeleteCourse;
