...
void HomepageWindow::UpdateCourseInfo(const QJsonArray& data)
{
	...
	++course_num;
    connect(ui_join_course_card_->join_course, &QPushButton::clicked, this, &HomepageWindow::OpenCreateCourseForm);
}
...
void HomepageWindow::OpenCreateCourseForm()
{
	...
	// 连接与加入课程表单有关的信号与槽
    connect(ui_join_course_form_->confirm, &QPushButton::clicked, this, &HomepageWindow::CreateCourse);
}

void HomepageWindow::CreateCourse()
{
	if (ui_join_course_form_) {
		QJsonObject data;
        QString course_name = ui_join_course_form_->course_id_input->text();
        data["course_name"] = course_name;

        Q_EMIT SendCreateCourse(data);
	}
}
